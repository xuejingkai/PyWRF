...
fig_height  = 5         # 图片高
fig_dpi     = 300       # 图片DPI
ver_num, hor_num   = 1, 3   # 一张图片显示子图的行数，列数，第几个子图，用于标记
title       = ["W(1000hPa) and Wind(10m) ","W(1000hPa) and Wind(10m) ","W(1000hPa) and Wind(10m) "]  # 子图的标题，务必用list形式表示，需要与子图数量对应
...
west_lon, east_lon      = 120.6, 122.2  # 图片显示范围，西侧和东侧经度
south_lat, north_lat    = 30.6, 32.0    # 图片显示范围，南侧和北侧纬度
...
ticklength  = 4     # 刻度线的高度
xlabellist  = ["default","modified_AH","modified_Building"]  # x轴的标签文字，不需要写为None，不论多少用list表示：["a", "b", "c", ...]，并且需要与子图数量对应
...
var_contourf    = 'wa'      # 填充的变量
var_contour     = 'rh2'     # 绘制等高线的变量
chem_w          = 0         # 摩尔质量，如果无需摩尔质量输入小于等于0的数字
u, v            = 'U10', 'V10'  # 经度方向和纬度方向的风速
loop_draw       = 0         # 1表示开启循环绘制，会按照下面的时间序列绘制多张图
timelist        = range(32, 176, 1)  # 时间列表，建议使用list格式，如果是连续时间也可以使用range方法,e.g.：range(56, 69, 2)
height_contourf = 1         # 填充数据高度，如果没有高度则随便填写一个数字
height_contour  = 0         # 轮廓线数据高度，如果没有高度则随便填写一个数字
height_quiver   = 0         # 轮廓线数据高度，如果没有高度则随便填写一个数字

# 填充图形信息设置
cmap    = cmaps.nrl_sirkes         # 填色的颜色类型，具体参考colormap与cmaps库
contourf_level  = np.arange(-0.6, 0.61, 0.03)      # 填色的最小值，最大值和间隔，如果要使用默认的则改成None
contourf_ticks  = np.arange(-0.6, 0.61, 0.06)       # 设置colorbar的ticks，如果不需要写成None
...
rectplace, rectextra        = 'bottom', 0.15  # 设置空白子图的位置（bottom，top，right，left），数值表示空白多少，相对子图的比例
rect1, rect2, rect3, rect4  = 0.1, 0.1, 0.8, 0.03    # colorbar的位置,分别是 水平位置，垂直位置，水平宽度，垂直宽度
hv_opt      = 'horizontal'        # 图例垂直还是水平,vertical或者horizontal
colorbar_labeltext  = '1000hPa处垂直风速(m/s)'      # 图例写什么字
...
# 图片保存信息设置
fig_path        = r"D:\Data\WRF-Chem_Files\WRF-Chem_Simulation\Pic\WRF-Chem模拟阶段1\compare_2"   # 图片保存路径
fig_name_base   = "ucm_compare"    # 图片保存的基础名
