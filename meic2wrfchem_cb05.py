...
def make_interp_meic_emission(emiss_year,emiss_month,md5value,lon_inp,lat_inp):
    ...
    if not flag:
        # 获取meic数据,并转化单位
        for spec in ['SO2','NOx','NH3','CO2','CO','OC','BC','PM25','PM10','CB05_ALD2','CB05_UNR','CB05_PAR','CB05_TOL','CB05_ETHA','CB05_TERP','CB05_NVOL'
            ,'CB05_ETH','CB05_MEOH','CB05_OLE','CB05_IOLE','CB05_ISOP','VOC','CB05_XYL','CB05_ETOH','CB05_FORM','CB05_ALDX']:
            interp_meic_emission[spec]={}
            for iemiss in ["agriculture","industry","power","residential","transportation"]:
                try:  #部分污染物仅存在于某些类型排放源
                    '''
                    if emiss_month[0]=="0":
                        emiss_month_one=emiss_month[-1]
                    else:
                        emiss_month_one=emiss_month
                    '''
                    meic_emis = np.loadtxt(CB05_DIR+"/"+emiss_year+emiss_month+"/"+emiss_year+"_"+emiss_month+"_"+iemiss+"_"+spec+".asc",skiprows = 6)[::-1,:]
                    logger.info(emiss_year+"_"+emiss_month+"_"+iemiss+"_"+spec+".asc readed")
                    ...
                    logger.info("error generating "+str(emiss_year)+"-"+str(emiss_month)+"-"+str(iemiss)+'-'+str(spec))
                    logger.info("error file: "+CB05_DIR+"/"+emiss_year+emiss_month+"/"+emiss_year+"_"+emiss_month+"_"+iemiss+"_"+spec+".asc")
...
def make_wrfchemi(x):
    ...
    if os.path.exists(ncfile) and os.path.getsize(ncfile) > 56700000:
        ...
        wrfchem_emission["E_ALD"]   =  interp_meic_emission['CB05_ALD2']["all"]["levels"] + interp_meic_emission['CB05_ALDX']["all"]["levels"]                #ald
        wrfchem_emission["E_HCHO"]  =  interp_meic_emission['CB05_FORM']["all"]["levels"]                                #hcho
        wrfchem_emission["E_ORA2"]  =  interp_meic_emission['CB05_NVOL']["all"]["levels"]                                #ora2 ##not sure
        wrfchem_emission["E_NH3"]   =  interp_meic_emission['NH3']["all"]["levels"]                                #nh3  
        wrfchem_emission["E_HC3"]   =  interp_meic_emission['CB05_PAR']["all"]["levels"]*0.2                            #hc3
        wrfchem_emission["E_HC5"]   =  interp_meic_emission['CB05_PAR']["all"]["levels"]*0.4                            #hc5
        wrfchem_emission["E_HC8"]   =  interp_meic_emission['CB05_PAR']["all"]["levels"]*0.6                            #hc8
        wrfchem_emission["E_ETH"]   =  interp_meic_emission['CB05_ETHA']["all"]["levels"]                                #eth
        wrfchem_emission["E_CO"]    =  interp_meic_emission['CO']["all"]["levels"]                               #co  
        wrfchem_emission["E_OL2"]   =  interp_meic_emission['CB05_ETH']["all"]["levels"]                                #ol2
        wrfchem_emission["E_OLT"]   =  interp_meic_emission['CB05_OLE']["all"]["levels"]                                #olt
        wrfchem_emission["E_OLI"]   =  interp_meic_emission['CB05_IOLE']["all"]["levels"]                                #oli
        wrfchem_emission["E_TOL"]   =  interp_meic_emission['CB05_TOL']["all"]["levels"]                                #tol
        wrfchem_emission["E_XYL"]   =  interp_meic_emission['CB05_XYL']["all"]["levels"]                                #xyl
        wrfchem_emission["E_KET"]   =  interp_meic_emission['CB05_ISOP']["all"]["levels"]*1.11                                    #ket
        wrfchem_emission["E_CSL"]   =  interp_meic_emission['CB05_ISOP']["all"]["levels"]*0.4+interp_meic_emission['CB05_OLE']["all"]["levels"]*0.3         #csl
        wrfchem_emission["E_ISO"]   =  interp_meic_emission['CB05_ISOP']["all"]["levels"]                                #iso
        ...
        #wrfchem_emission["E_PM_10"] =  interp_meic_emission['PM25']["all"]["levels"]+interp_meic_emission['PMcoarse']["all"]["levels"]                #pm10
        wrfchem_emission["E_PM_10"] =  interp_meic_emission['PM10']["all"]["levels"]                #pm10
...
def parallel_make_wrfchemi(start_time,end_time,n_jobs=-1):
    ...
    if n_jobs == -1:
        ...
        logger.info("Will use "+str(n_jobs)+" core to compute.")
    ...
    for itime in timelist:
        ...
        if not  itime_bjt.strftime("%m")  in   month_already:
            if int(itime_bjt.strftime("%Y"))>2017:
                ...
                make_interp_meic_emission(itime_bjt.strftime("%Y"), itime_bjt.strftime("%m"), md5value, lon_inp,lat_inp)
...
if __name__ == '__main__':
    start_time = datetime(2016,7,21,0)
    end_time = datetime(2016,7,23,0)
