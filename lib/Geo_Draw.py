...
class Figure4wrf():
    ...
    def geo_draw(self,lake_opt,lake_linewidth,lake_linecolor,coastline_linewidth,coastline_color,precision):
        # 添加海岸线数据
        self.axe.add_feature(cfeat.COASTLINE.with_scale(precision), linewidth=coastline_linewidth,color=coastline_color,zorder=1)
        ...
        if lake_opt==0:
            ...
            self.axe.add_feature(LAKES_border, linewidth=lake_linewidth,zorder=1)
            print("绘制湖泊")
        if lake_opt==1:
            print("不绘制湖泊")
    ...
    def contourf_draw(self,x,y,factor,cmap,level,contourf_opt=0,extend="neither"):
        if contourf_opt==0:
            self.contourf = self.axe.contourf(x, y, factor, levels=level, cmap=cmap,extend=extend,zorder=0)
        if contourf_opt==1:
            self.contourf = self.axe.contourf(x, y, factor, cmap=cmap,extend=extend,zorder=0)
        print("最大和最小的值分别是：")
