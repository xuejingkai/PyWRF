...
title       = ["Temperature(2m) and Wind(10m) ","Temperature(2m) and Wind(10m) ","Temperature(2m) and Wind(10m) "]  # 子图的标题，务必用list形式表示，需要与子图数量对应
...
height_contourf = 0         # 填充数据高度，如果没有高度则随便填写一个数字
...
contourf_level  = np.arange(25, 40.1, 0.3)  # 填色的最小值，最大值和间隔，如果要使用默认的则改成None
contourf_ticks  = np.arange(25, 40.1, 1)  # 设置colorbar的ticks，如果不需要写成None
...
colorbar_labeltext  = '2m温度(^oC)'      # 图例写什么字
...
# 图片保存信息设置
fig_path        = r"D:\Data\WRF-Chem_Files\WRF-Chem_Simulation\Pic\compare_1"   # 图片保存路径
